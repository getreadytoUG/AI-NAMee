                    ...
                    if wandb_run:
                        if not train_config.enable_fsdp or rank==0:
                            wandb_run.log({
                                ...
                                'train/learning_rate': optimizer.param_groups[0]['lr'], # add learning rate log
                            })

                    pbar.set_description(f"Training Epoch: {epoch+1}/{train_config.num_epochs}, step {step}/{len(train_dataloader)} completed (loss: {loss.detach().float()}, lr: {optimizer.param_groups[0]['lr']})")
